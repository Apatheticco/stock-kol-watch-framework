...
### 3.3 建你的关注名单（最重要的一步）
框架**内置一个 5 人公开参考 starter**（⚠️ 示例非推荐/非投资建议，开箱即跑）。新手直接复制 `references/example-roster.md`（含这 5 人样板 + 按角色配齐模板 + 怎么找账号），填好存到 `$KOL_VAULT/references-roster.md`（🅰️ 模式从这里读名单；🅱️ 模式直接告诉 Claude）。**这 5 个偏半导体、没唱空账号——正式用前记得补一个唱空/质疑账号。** 评级/画像方法见 `references/account-roster.md`。
...
这是核心。框架靠你喂这几类信息运转，**喂得越准，结论越可靠**：

| 信息 | 必填? | 什么时候给 | 例子 |
|------|------|-----------|------|
| **关注名单 roster** | ✅ 必填 | 首次配置 | "我要关注 @账号A @账号B … 这 N 个" |
| **持仓** | 持仓监控需要 | 首次 + 每次买卖后 | "我持有 $AAAA 10 股，成本 $50；$BBBB 100 股 @ $12"（虚构示例）|
| **现金口径** | ⚠️ 算 Risk Budget 必需 | 首次 + 变动时 | "现金 $X + 货币基金 $Y"（**要含近现金，不只交易账户余额**）|
| **时区 / 窗口偏好** | 有默认 | 首次（或临时） | "我在 UTC+8，按自然日切" / 临时"拉最近 12h" |
| **买卖动作** | 持仓准确性靠它 | 每次成交后立刻 | "我买了 $CCCC 20 股 @ $30"（虚构示例）|
| **数据源凭证** | ✅ | 装 MCP 时 | followin（或你用的 MCP）的连接 |
| **你的 thesis（可选）** | 否 | 想让它追踪某个判断时 | "我赌存储被重定价为成长股，帮我盯证据" |

**铁律**：
- **持仓数量/成本只来自你口头告知**（框架不接券商）。你交易忘说 → 浮盈、占比、thesis 全链连锁错且静默无告警。**养成"成交就说一句"的习惯。**
- 现金一定报全口径（含货基/账户外现金），否则它**拒绝**给你算 Risk Budget 红线（宁可不报也不编分母）。
